...
class RealmPlaygroundTests(ZulipTestCase):
    ...
    def test_delete_realm_playground(self) -> None:
        ...
        playground_id = do_add_realm_playground(realm, payload)
        self.assertTrue(RealmPlayground.objects.filter(name="Python").exists())

        result = self.api_delete(iago, f"/json/realm/playgrounds/{playground_id + 1}")
        self.assert_json_error(result, "Playground does not exist", status_code=404)
